...
from .serializers import ProductViewSet, OneProductViewSet
...
def ProductView(request):
    ...
    sterilized = OneProductViewSet (query_set, many=False)
    return Response(sterilized.data)

@api_view(['POST'])
def createOneProductView (request):
    serialized = OneProductViewSet(data=request.data)
...
def updateOneProductView(request, id):
    serialized = OneProductViewSet(instance=Product.objects.get(id=id), data=request.data) #instance specifies product
    if serialized.is_valid():
        serialized.save()
    else:
        print("Invlaid Data")
